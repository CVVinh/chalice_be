...
from api.models.models import AccountMaster, InsurancesMaster, OptionsMaster, RentalOrderCart, VehiclesMaster
...
message_rental_order_cart_constant = MessageResponse()
message_rental_order_cart_constant.setName("Rental Order Cart Master")
...
def add_rental_order_cart(rental_order_cart_obj):
    ...
    session.commit()
    return (True, message_rental_order_cart_constant.MESSAGE_SUCCESS_CREATED)
...
def update_rental_order_cart_info(rental_order_cart_obj):
    ...
    if (
        update_rental_order_cart := session.query(RentalOrderCart).filter(
            RentalOrderCart.rentalOrdersCartId == rental_orders_cart_id, RentalOrderCart.isDeleted == 0).first()
    ):
        add_update_object(rental_order_cart_obj, update_rental_order_cart)
        session.commit()
        return (True, message_rental_order_cart_constant.MESSAGE_SUCCESS_UPDATED)
    return (False, message_rental_order_cart_constant.MESSAGE_ERROR_NOT_EXIST)
...
def delete_soft_rental_order_cart(query_params):
    ...
    if (
        ...
        session.commit()
        return (True, message_rental_order_cart_constant.MESSAGE_SUCCESS_DELETED)
    return (False, message_rental_order_cart_constant.MESSAGE_ERROR_NOT_EXIST)
...
def delete_hard_rental_order_cart(query_params):
    ...
    rental_orders_cart_id = query_params.get("rentalOrdersCartId")
    if (
        delete_rental_order_cart := session.query(RentalOrderCart).filter(
            RentalOrderCart.rentalOrdersCartId == rental_orders_cart_id, RentalOrderCart.isDeleted == 0).first()
    ):
...
def delete_soft_multi_rental_order_cart(query_params):
    ...
    for item in query_params:
        if (
            update_rental_order_cart := session.query(RentalOrderCart).filter(
                RentalOrderCart.rentalOrdersCartId == item, RentalOrderCart.isDeleted == 0).first()
        ):
            update_rental_order_cart.isDeleted = 1
            update_rental_order_cart.deletedAt = func.now()
    session.commit()
...
def get_rental_order_cart_info(query_params):
    ...
    Returns:
        Response: Returning a message and a object inclue obj the rental_order_cart
    """
    filter_param_get_list = filter_param_get_list_rental_order_cart(
        query_params)

    if (len(filter_param_get_list) > 0):
        return (True, {"mstRenOrdCartModel": filter_param_get_list,
                       "total": len(filter_param_get_list),
                       "message": message_rental_order_cart_constant.MESSAGE_SUCCESS_GET_INFO,
                       "status": 200})
    return (False, message_rental_order_cart_constant.MESSAGE_ERROR_NOT_EXIST)
...
def filter_param_get_list_rental_order_cart(query_params):
    ...
    """
    query_list_rental_order_cart = session.query(
        RentalOrderCart).filter(RentalOrderCart.isDeleted == 0)

    parameters = ["rentalOrdersCartId", "accountId",
                  "vehicleId", "optionId", "insuranceId", "statusCart"]
    if query_params:
        for param in parameters:
            if (param in query_params):
                query_list_rental_order_cart = query_list_rental_order_cart.filter(
                    getattr(RentalOrderCart, param) == query_params[param])

    convert_arr_rental_order_cart = [object_as_dict(
        item) for item in query_list_rental_order_cart.all()]
...
def handlerInclueData(list_data):
    ...
    sorted_data = sorted(list_data,
                         key=lambda x: (x['accountId'], x['vehicleId']))
    ...
    for key, group in grouped_data:
        ...
        obj_account = session.query(AccountMaster).filter(
            AccountMaster.isDeleted == 0, AccountMaster.accountId == code[0]).first()
        obj_vehical = session.query(VehiclesMaster).filter(
            VehiclesMaster.vehicleId == code[1]).first()
        obj_option = []
        obj_insurances = []

        for item in item_of_list:
            if item['optionId'] is not None:
                list_options = session.query(OptionsMaster).filter(
                    OptionsMaster.optionId == item['optionId']).first()
                obj_option.append(list_options)

            if item['insuranceId'] is not None:
                list_insurances = session.query(InsurancesMaster).filter(
                    InsurancesMaster.insuranceId == item['insuranceId']).first()
