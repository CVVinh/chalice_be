...
class AccountMaster(Base):
    ...
    accountCd = Column(
        "account_cd", String(100), comment="アカウントコード:社員番号など利用企業のIDやコードのため"
    )
    extAccountId = Column(
        "ext_account_id", Integer, comment="外部アカウントID:cognitoなどの外部認証サービスと連携する場合は設定"
    )
...
class AccountBaseMaster(Base):
    ...
    accountId = Column(
        "account_id", ForeignKey("m_accounts.account_id"), comment="アカウントID"
    )
    baseId = Column("base_id", ForeignKey("m_base.base_id"), comment="拠点ID")
    version = Column(
        "version",
        Integer,
        nullable=False,
    ...
    createdBy = Column(
        "created_by",
        ForeignKey("m_accounts.account_id"),
...
class BaseMaster(Base):
    ...
    prefCode = Column(
        "pref_code",
        ForeignKey("m_prefecture.pref_id"),
        nullable=False,
        comment="都道府県コード",
    )
...
class VehiclesMaster(Base):
    ...
    vehicleName = Column("vehicle_name", String(
        200), comment="名前車両", nullable=False)
    makerId = Column(
        "maker_id",
        Integer,
        ForeignKey("m_makers.maker_id"),
        comment="メーカーID",
        nullable=False,
    )
    storeId = Column(
    ...
    vehicleStatus = Column(
        "vehicle_status", Integer, comment="車の状態：1-利用可能、2-レンタル済、3-保守中"
    )
    vehicleSeat = Column(
        "vehicle_seat", Integer, comment="Số lượng ghế", nullable=False
    )
    vehicleModel = Column(
        "vehicle_model",
        Integer,
        ForeignKey("m_models.model_id"),
        comment="Kiểu xe",
        nullable=False,
    )
    vehicleValue = Column("vehicle_value", Float,
                          comment="Giá xe", nullable=False)
...
class OptionsMaster(Base):
    ...
    optionId = Column("option_id", Integer,
                      primary_key=True, comment="オプションID")
...
class InsurancesMaster(Base):
    ...
    insuranceId = Column("insurance_id", Integer,
                         primary_key=True, comment="保険ID")
...
class PaymentMethodsMaster(Base):
    ...
    paymentMethodId = Column(
        "payment_method_id", Integer, primary_key=True, comment="お支払い方法ID"
    )
    paymentMethodName = Column(
        "payment_method_name", String(200), comment="名前お支払い方法")
...
class RentalOrders(Base):
    ...
    rentalOrdersId = Column(
        "rental_orders_id", Integer, primary_key=True, comment="発注明細ID:自動採番"
    )
    ...
    createdBy = Column("created_by", ForeignKey(
        "m_accounts.account_id"), comment="作成者")
    modifiedAt = Column(
        "modified_at",
        DateTime,
        nullable=False,
    ...
    modifiedBy = Column(
        "modified_by", ForeignKey("m_accounts.account_id"), comment="更新者"
    )
    deletedAt = Column("deleted_at", DateTime, comment="削除日時")
    deletedBy = Column("deleted_by", ForeignKey(
        "m_accounts.account_id"), comment="削除者")
...
class RentalOrderDetail(Base):
    ...
    vehicleId = Column(
        "vehicle_id",
        ForeignKey("m_vehicles.vehicle_id"),
        nullable=False,
        comment="車両ID",
    )
    optionId = Column("option_id", String(200), nullable=False, comment="車両ID")
    insuranceId = Column(
        "insurance_id", String(200), nullable=False, comment="ID bao hiem"
    )
    quantity = Column("quantity", Integer, comment="数量", nullable=False)
    amount = Column("amount", Float, comment="小計", nullable=False)
    rentalStartDate = Column(
        "rental_start_date", DateTime, nullable=False, comment="レンタル開始日"
    )
    rentalEndDate = Column(
        "rental_end_date", DateTime, nullable=False, comment="レンタル終了日"
    )
    createdAt = Column(
        "created_at",
        DateTime,
...
class RentalOrderCart(Base):
    ...
    rentalOrdersCartId = Column(
        "rental_orders_cart_id", Integer, primary_key=True, comment="発注明細ID:自動採番"
    )
    accountId = Column(
        "account_id", ForeignKey("m_accounts.account_id"), comment="アカウントID"
    )
    ...
    optionId = Column("option_id", ForeignKey(
        "m_options.option_id"), comment="車両ID")
    insuranceId = Column(
        "insurance_id", ForeignKey("m_insurances.insurance_id"), comment="保険ID"
    )
    statusCart = Column(
        "status_cart",
        Integer,
