...
@payment_method_bp.route("/func/get-payment-method-list", methods=["POST"])
@errors_handle
@transaction()
def get_payment_method_list_controller():
    request = payment_method_bp.current_request.json_body
    if request is not None and "params" in request:
        request = request["params"]
...
@payment_method_bp.route("/func/export-payment-method-list", methods=["POST"])
@errors_handle
@transaction()
